...
!!! NCL = 2, dirichlet BC
       ...
       if (istret==0) then
          ...
          enddo
          do is = 1, 3
             aamB(:,is) = one/pp2y -xcstB*aamB(:,is)
          enddo
       endif
       ! BC on aam, dirichlet
       aam(1 )=one
       ...
       do is = 1, 3
          aamB(1,is)=one !todo -- not this for partally conducting?
          aamB(ny,is)=one
       enddo
       ...
       if (istret==0) then
          ...
          enddo
          do is = 1, 3
             bbmB(2     ,is)=bbmB(2     ,is)+alsa2y
             bbmB(ny-1  ,is)=bbmB(ny-1  ,is)+alsamy
             bbmB(3     ,is)=bbmB(3     ,is)+alsa3y
             bbmB(ny-2  ,is)=bbmB(ny-2  ,is)+alsaty
             bbmB(4:ny-3,is)=bbmB(4:ny-3,is)+alsajy
          enddo
          ...
          enddo
          do is = 1, 3
             bbmB(2     ,is)=bbmB(2     ,is)+alsa2y/pp2y(3)
             bbmB(ny-1  ,is)=bbmB(ny-1  ,is)+alsamy/pp2y(ny)
             bbmB(3     ,is)=bbmB(3     ,is)+alsa3y/pp2y(4)
             bbmB(ny-2  ,is)=bbmB(ny-2  ,is)+alsaty/pp2y(ny-1)
             bbmB(4:ny-3,is)=bbmB(4:ny-3,is)+alsajy/pp2y(5:ny-2)
          enddo
       ...
       if (istret==0) then
          ...
          enddo
          do is = 1, 3
             ddmB(2     ,is)=ddmB(2     ,is)+alsa2y/pp2y(1)
             ddmB(ny-1  ,is)=ddmB(ny-1  ,is)+alsamy/pp2y(ny-2)
             ddmB(3     ,is)=ddmB(3     ,is)+alsa3y/pp2y(2)
             ddmB(ny-2  ,is)=ddmB(ny-2  ,is)+alsaty/pp2y(ny-3)
             ddmB(4:ny-3,is)=ddmB(4:ny-3,is)+alsajy/pp2y(3:ny-4)
          enddo
